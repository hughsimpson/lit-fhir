...
object ScalaCodegen extends BaseFieldImplicits with Commonish {
  ...
  def scala2EnumDeclaration(forceDisplayName: Boolean = false)(valueSet: String, codes: CodeValueSet): String = {
    ...
    s"""
       ...
       |  def toCoding: Option[Coding] = system.map(s => Coding(system = Some(s), code = Some(entryName), display = display.map(toFHIRString)))
       |}
  ...
  def genScala2Enums(pkg: String, valueSets: Map[String, CodeValueSet]): String = {
    ...
    s"""package com.babylonhealth.lit.$pkg
       ...
       |import com.babylonhealth.lit.core.{ EnumeratumBase, FhirEnum, FhirCirceEnum, EnumWithFallback, toFHIRString }
       |
       |$enumDecl
       |""".stripMargin
  }
